...
const CATEGORIAS = {
  gondola: "Gôndola",
  produto_solto: "Produto Solto",
  fardo: "Fardo",
  ponto_extra_refri: "Ponto Extra Refri",
  ponto_extra_cerveja: "Ponto Extra Cerveja",
};
    ...
    .map((p) => {
      ...
      return `
        <div class="tag status-${status}">
          ...
          <div class="tag-meta">
            <span class="cat-chip">${CATEGORIAS[p.categoria] || p.categoria || "—"}</span>
...
function entrarModoEdicaoProduto(p) {
  ...
  $("#p_nome").value = p.nome;
  $("#p_categoria").value = p.categoria || "produto_solto";
...
$("#formProduto").addEventListener("submit", async (e) => {
  ...
  if (editandoProdutoId) {
    const { error } = await sb
      .from("produtos")
      .update({ nome, categoria, quantidade, validade })
      .eq("id", editandoProdutoId);
    if (error) return toast("Erro ao atualizar produto");
    toast("Produto atualizado ✅");
    sairModoEdicaoProduto();
  } else {
    const { error } = await sb.from("produtos").insert({ nome, categoria, quantidade, validade });
...
$("#btnGerarWpp").addEventListener("click", () => {
  const filtro = $("#wpp_filtro").value;
  const categoriaFiltro = $("#wpp_categoria").value;
  ...
  if (filtro === "alerta") {
    ...
    const diasLimite = parseInt(filtro, 10);
    lista = lista.filter((p) => getStatus(p.validade).dias <= diasLimite);
  }
  ...
  if (lista.length === 0) {
    toast("Nenhum produto encontrado para esse filtro");
    return;
  }
  ...
  const labelFiltro = {
    ...
    alerta: "Vencidos / em atenção",
    30: "Vencendo em até 30 dias",
    60: "Vencendo em até 60 dias",
    90: "Vencendo em até 90 dias (3 meses)",
    180: "Vencendo em até 6 meses",
    365: "Vencendo em até 1 ano",
  }[filtro];

  let texto = `📦 *CONTROLE DE ESTOQUE*\n🗓️ ${hoje}\n🔎 ${labelFiltro}`;
  if (categoriaFiltro !== "todas") texto += ` — ${CATEGORIAS[categoriaFiltro]}`;
  texto += `\n━━━━━━━━━━━━━━━━━━━━\n\n`;

  lista.forEach((p) => {
    const { status, dias } = getStatus(p.validade);
    texto += `${emoji[status]} *${p.nome}*\n`;
    texto += `   Categoria: ${CATEGORIAS[p.categoria] || p.categoria}\n`;
